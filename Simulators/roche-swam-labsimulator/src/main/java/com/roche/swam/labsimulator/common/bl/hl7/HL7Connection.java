...
public class HL7Connection  {
	...
	private String host = MainApp.getFilepath(MainApp.getDeviceName()+"HostName");
	private int port = Integer.parseInt(MainApp.getFilepath(MainApp.getDeviceName()+"AdapterPort"));
	...
	public void run(MessageSender sender, SampleRepository samples, EnumSampleStatus sampleStatus) {
		try {
			...
			LOGGER.info(" HL7Connection connection status:" + this.client.isConnected());
			if (this.client.isConnected()) {

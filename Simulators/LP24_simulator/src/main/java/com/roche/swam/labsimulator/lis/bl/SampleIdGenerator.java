...
public class SampleIdGenerator {
	...
	public static List<String> getLpSamplePosition() {
	...
	public static  List<String> getLp24dpcrChipContainer(){
		List<String> container = new ArrayList<>();
		for(int i=0;i<12;i++) {
			container.add(getNext());
		}
		return container;
	...
	public static  List<String> getLp24Chip(){
		List<String> container = getLp24dpcrChipContainer();
		List<String> containerPosition=  new ArrayList<>(); 
		for(int i=0; i<container.size();i++) {
		for(int j=1;j<=8;j++) {
			
			containerPosition.add(container.get(i)+'_'+j);
		}
		}

...
import { NOTFSEVERITY, NTROUTERURLS, TosterClasses, multipleNotfMsg } from '../utils/notification.util';
...
export class NotificationtoasterService {
  ...
  constructor(
    ...
    private _adminAPIS: AdminApiService,
    private _httpClient: HttpClient,
    private _router: Router
  ) { }

  public getNewNotification() {
    const appProperties = this._SS.getSharedData('appProperties');
    // tslint:disable-next-line:max-line-length
    const URL = `${appProperties.protocol}://${appProperties.host}:${appProperties.admmApi.port}/${appProperties.admmApi.module}/json/rest/api/v1/notification`;
    // tslint:disable-next-line:max-line-length
    return this._adminAPIS.getByInterval(this._httpClient.get(URL, { withCredentials: true}).pipe(catchError(this._adminAPIS.handleError)), true);
  }
  ...
  public loadNotifToster() {
    setTimeout(() => {
      ...
      if (this.isToasterNeeded()) {
        const apiProp = this._SS.getSharedData('appProperties');
  ...
  public showNotfToster(notfObj: any): void {
    ...
    this._snackBar.openFromComponent(NotificationtoasterComponent, {
      ...
      const isToasterClosed = this._SS.getSharedData('isToasterClosed');
      this._SS.setSharedData('isToasterClosed', false);

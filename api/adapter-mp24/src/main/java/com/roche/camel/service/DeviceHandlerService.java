...
public class DeviceHandlerService {
	...
	public boolean updateDeviceStatus(String serialNo, String status) {
		...
		if (deviceFlag) {
			...
			String resp = builder2.get(String.class);
				...
				registeredStatus = deviceStatusValidation(serialNo, status, token, resp);
    ...
    public boolean deviceStatusValidation(String serialNo, String status, String token, String resp) {
        ...
        try {
        	...
        	if (respobj.length() == 0) {
        		...
        		AdmNotificationService.sendNotification(NotificationGroupType.UNREGISTERED_DEVICE_MP24,content,token,admNotificationURL);
        		throw new HL7Exception("serial number mismatch");
        	}
        	...
        	if (!(attributes.getString(PROTOCOL_VERSION).equalsIgnoreCase(protocolVersion) || attributes.getString(PROTOCOL_VERSION).equals(""))) {
        		List<String> content=new ArrayList<>();
        		content.add(serialNo);
        		AdmNotificationService.sendNotification(NotificationGroupType.INVALID_HL7_VER_MP24,content,token,admNotificationURL);
        		throw new HL7Exception("HL7 protocol version mismatch");
        	}
        	if (!(deviceType.equalsIgnoreCase(getDeviceModel()))) {
        		List<String> content=new ArrayList<>();
        		content.add(serialNo);
        		AdmNotificationService.sendNotification(NotificationGroupType.INVALID_DEVICE_MODEL_MP24,content,token,admNotificationURL);
        ...
        deviceSubStatusValidation(serialNo, attributes);
        	...
        	if(status.equalsIgnoreCase(OFFLINE)) {
        		List<String> content=new ArrayList<>();
        		content.add(serialNo);
        		AdmNotificationService.sendNotification(NotificationGroupType.DEVICE_OFFLINE_MP24,content,token,admNotificationURL);
        	}
        } catch (Exception e) {
	...
	public void deviceSubStatusValidation(String serialNo, JSONObject attributes) {
		try {
			String oldDeviceSubStatus = Optional.ofNullable(String.valueOf(attributes.get("deviceSubStatus")))
					.orElse(null);

			if ("OP".equalsIgnoreCase(oldDeviceSubStatus) && "ID".equalsIgnoreCase(deviceSubStatus)) {
				String updateIMMRunResultURL = immUrl + SOURCE_AND_DEVICETYPE + DeviceType.MP24 + MESSAGE_TYPE

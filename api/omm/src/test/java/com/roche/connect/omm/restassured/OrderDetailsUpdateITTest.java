/*******************************************************************************
 * File Name: OrderDetailsUpdateTest.java            
 * Version:  1.0
 * 
 * Authors: Ankit Singh
 * 
...
/**
 * 
 * @author imtiyazm This class is used to test updation for OrderDetails info
 */
public class OrderDetailsUpdateITTest {
	String jsonContent = "";
	...
	public static final String correctJsonFile = "src/test/java/resource/OrderCrudUpdateDetails.json";
	public static final String IncorrectJsonFile = "src/test/java/resource/OrderCrudUpdateWrongDetails.json";
	...
	/**
	 * update OrderDetails info test with correct json.
	 *
	 * @throws IOException Signals that an I/O exception has occurred.
	...
	@Test(priority = 1)
	public void testUpdateOrderDetailsForCorrectJson() throws IOException, HMTPException {
		jsonContent = JsonFileReaderAsString.getJsonfromFile(correctJsonFile);
		...
		Response resp = requestSpecification.contentType("application/json").body(jsonContent).when().put(api);
	...
	/**
	 * update OrderDetails info test with incorrect json.
	 *
	 * @throws IOException Signals that an I/O exception has occurred.
	 * @throws HMTPException the HMTP exception
	 */
	@Parameters({ "OrderManagementAPITest" })
	@Test(priority = 2)
	public void testUpdateOrderDetailsForInCorrectJson() throws IOException, HMTPException {
		jsonContent = JsonFileReaderAsString.getJsonfromFile(IncorrectJsonFile);
		RequestSpecification requestSpecification = RestAssured.given().relaxedHTTPSValidation().redirects().follow(false).cookie(cookieKey,token);
		Response resp = requestSpecification.contentType("application/json").body(jsonContent).when().put(api);
		int statusCode = resp.getStatusCode();
		Assert.assertNotEquals(statusCode, expectedValue);

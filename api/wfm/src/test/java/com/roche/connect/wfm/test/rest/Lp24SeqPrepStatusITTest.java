...
public class Lp24SeqPrepStatusITTest {

	String jsonContent = "";
	public static final String jsonForSucessWfmCreation = "src/test/java/resource/Lp24SeqPrepStatusPositive.json";
	public static final String jsonForFailureWfmCreation = "src/test/java/resource/Lp24SeqPrepStatusNegative.json";
	...
	@Test(priority = 1)
	public void PositiveLp24SeqPrepStatusTest() throws IOException {
		...
		Response resp = RestAssured.given().cookie(cookieKey, token).contentType("application/json").body(jsonContent).when()
				.post("/updatewfmprocess/LPStatus");
		...
		//Assert.assertEquals(statusCode, 200, "wfm has recieved Lp24SeqPrep-u03 successfully");

	}

	@Test(priority = 2)
	public void NegativeLp24SeqPrepStatusTest() throws IOException {
		...
		//Assert.assertNotEquals(statusCode, 200, "wfm Error occured while receiving Lp24SeqPrep-u03");

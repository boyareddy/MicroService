/*******************************************************************************
 * UpdateLP24PrePcrResult.java Version: 1.0 Authors: somesh_r
 ...
 * to update LP24 Pre Pcr results. *********************
 ******************************************************************************/
...
/**
 * Class implementation to update LP24 Pre Pcr results.
 */
public class UpdateLP24PrePcrResult implements JavaDelegate {
	...
	public void execute(DelegateExecution execution) {
		...
		try {
			...
			String orderStatusPrePcrUpdate = (String) execution
					.getVariable(WfmConstants.WORKFLOW_VARIABLES.ORDERSTATUSPREPCRUPDATE.toString());
			...
			String accessioningId = (String) execution
					.getVariable(WfmConstants.WORKFLOW_VARIABLES.ACCESSIONINGIDPREPCRUPDATE.toString());
			String assayType = (String) execution.getVariable(WORKFLOW_VARIABLES.ASSAYTYPE.toString());
			
			@SuppressWarnings("unchecked")
			List<WfmDTO> updateOrderDetails = (List<WfmDTO>) execution
					.getVariable(WfmConstants.WORKFLOW_VARIABLES.ORDERDETAILSPREPCRUPDATE.toString());

			if (updateOrderDetails != null) {
				updateOrderDetails.stream().forEach(orderDetail -> {
					orderDetail.setAccessioningId(accessioningId);
					orderDetail.setDeviceId((String) execution
							.getVariable(WfmConstants.WORKFLOW_VARIABLES.DEVICEIDPREPCRUPDATE.toString()));
					orderDetail.setSampleType(
							(String) execution.getVariable(WfmConstants.WORKFLOW_VARIABLES.SAMPLETYPE.toString()));
					orderDetail.setInputContainerId((String) execution
							.getVariable(WfmConstants.WORKFLOW_VARIABLES.TUBECONTAINERIDPREPCR.toString()));
					orderDetail.setProtocolname((String) execution
                        .getVariable(WfmConstants.WORKFLOW_VARIABLES.PROTOCOLDPCRLP24.toString()));
					orderDetail.setAssayType(assayType);
					orderDetail.setInputposition((String) execution
							.getVariable(WfmConstants.WORKFLOW_VARIABLES.TUBECONTAINERPOSITIONPREPCR.toString()));
					orderDetail.setSendingApplicationName(
							activityProcessData.getSpecimenStatusUpdateMessage().getSendingApplicationName());
					orderDetail.setMolecularId((String) execution.getVariable("molicularId"));

					if (orderStatusPrePcrUpdate.equalsIgnoreCase(WORKFLOW_STATUS.PASSED.toString())) {
						orderDetail.setOrderStatus(WORKFLOW_STATUS.COMPLETED.toString());
					} else if (orderStatusPrePcrUpdate.equalsIgnoreCase(WfmConstants.ORDER_STATUS.ABORTED.toString())) {
						orderDetail.setOrderStatus(WfmConstants.ORDER_STATUS.ABORTED.toString());
					} else if (orderStatusPrePcrUpdate.equalsIgnoreCase(WORKFLOW_STATUS.INPROGRESS.toString())) {
